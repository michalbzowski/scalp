...
public class SyncAPIConnector extends StreamingConnector {
    ...
    private Long lastCommandExecutionTimestamp;
    ...
    public synchronized String safeExecuteCommand(BaseCommand cmd) throws APICommunicationException {
        ...
        if (lastTimeExecuted == null) {
            ...
            long msAfterLastCommand = cal.getTimeInMillis() - lastCommandExecutionTimestamp;
            if (msAfterLastCommand < 200L && msAfterLastCommand > timeToWait) {
                timeToWait = msAfterLastCommand;
            }
        }
        if (timeToWait > 0L) {
            try {
        ...
        this.lastCommandExecutionTimestamp = cal.getTimeInMillis();
        return result;
    }
